...
/**
 * @author Braedon Wooding, and @Dong Ao
 */
public class Checker implements CheckerStrategy{
    ...
    public boolean isMad() { return mad; }

    public void setMad() { this.mad = true; }
    ...
    public List<Position> validPositions(Checkerboard board, Position position) {
        ...
        if (color.equals(CheckerColor.RED)) {
            addValidPositions(board, position, positions, position.upRight());
            addValidPositions(board, position, positions, position.upLeft());
            if (crowned) {
                addValidPositions(board, position, positions, position.downLeft());
                addValidPositions(board, position, positions, position.downRight());
            }
    ...
    @Override
    public void addValidPositions(Checkerboard board, Position position,
                                  List<Position> positions, Position nextPosition) {
        if (board.isInBounds(nextPosition)) {
            Checker nextChecker = board.getPieceAt(nextPosition);
            if (nextChecker == null) {
                positions.add(nextPosition);
            } else if (!nextChecker.getColor().equals(color) || mad) {
                Position nextNextPosition = null;
                if (nextPosition.equals(position.upLeft())) {
                    nextNextPosition = nextPosition.upLeft();
                }
                else if (nextPosition.equals(position.upRight())) {
                    nextNextPosition = nextPosition.upRight();
                }
                else if (nextPosition.equals(position.downLeft())) {
                    nextNextPosition = nextPosition.downLeft();
                }
                else if (nextPosition.equals(position.downRight())) {
                    nextNextPosition = nextPosition.downRight();
                }
                if (board.isInBoundsAndEmpty(nextNextPosition)) {
                    positions.add(nextNextPosition);
                }
            }
        }
    }
