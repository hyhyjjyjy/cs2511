...
public class Checkerboard {
    ...
    public void moveChecker(Position position, Position endPosition) {
        if (isInBounds(position) && isInBounds(endPosition)) {
            ...
            if (!(position.upLeft().equals(endPosition)
            || position.upRight().equals(endPosition)
            || position.downLeft().equals(endPosition)
            || position.downRight().equals(endPosition))) {
                if (board[endPosition.getCol()][endPosition.getRow()].isMad()
                        && board[endPosition.getCol()][endPosition.getRow()].getColor()
                        == board[midPoint.getCol()][midPoint.getRow()].getColor()) {
                    board[midPoint.getCol()][midPoint.getRow()].setMad();
                }
                else {
